...
describe("Eventing", function () {
  ...
  before(async () => {
    const EventingFactory = await ethers.getContractFactory("Eventing");

    contract = await EventingFactory.deploy();
  ...
  it("should can buy some ticket", async () => {
    ...
    expect(ticketDetails.ticket.row).equal(BigNumber.from(4));
    expect(ticketDetails.ticket.seet).equal(BigNumber.from(25));
  });

  it("should use ticket correctly", async () => {
    const [owner, addr1, addr2] = await ethers.getSigners();

    const ticketIdForUse = 1;
    ...
    expect(ticketDetails.ticket.enable).equals(false);
